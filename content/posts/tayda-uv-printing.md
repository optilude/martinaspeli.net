...
The "GLOSS-V" (shiny varnish) or "GLOSS-M" (matte) layer can be made in a similar way. Copy everything you want to make glossy, paste it into the gloss layer, and change the color to `RDG_GLOSS`, which will look a blueish gray and will cover the "COLOR" layer contents fully unless you choose to skip gloss on some parts of it. (Conincidentally, I've wondered if it would be a good idea to just cover the whole design with a gloss square to coat all of it, but I believe this would leave slight height differences across the design and it just doesn't seem to be the way others have done it, so I never tried.)
